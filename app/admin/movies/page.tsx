...
import { Pencil, Plus, Search, Trash2, Film, Tv, ChevronRight, LayoutGrid, Sparkles, AlertCircle, Calendar, Filter, ArrowUpDown, Star } from "lucide-react";
...
export default function AdminMoviesPage() {
  ...
  const totalMovies = filteredItems.length;

  const deleteContent = async (id?: string) => {
    if (!id) return;
    const confirmed = window.confirm("Delete this movie? This action cannot be undone.");
    if (!confirmed) return;
    setStatus("Deleting...");
  ...
  const bulkDelete = async () => {
    ...
    const confirmed = window.confirm(`Delete ${selectedItems.size} movies? This action cannot be undone.`);
    if (!confirmed) return;
    setStatus("Deleting...");
    ...
    await loadContent();
    setStatus("Movies deleted successfully.");
  };
      ...
      <div className="flex min-h-[80vh] items-center justify-center">
        <div className="w-full max-w-md">
          <div className="mb-10 text-center">
            ...
            <h1 className="font-[var(--font-heading)] text-4xl font-bold text-white">Movies</h1>
            <p className="mt-3 text-gray-400">Enter your credentials to manage movies</p>
          </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="font-[var(--font-heading)] text-3xl font-bold text-white">Movies</h1>
          <p className="mt-1 text-sm text-gray-500">{totalMovies} movies in your library</p>
        </div>
        <div className="flex gap-3">
          <Link
            href="/admin/add/movie"
            className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-red-600 to-red-700 px-5 py-2.5 text-sm font-bold text-white transition-all hover:from-red-500 hover:to-red-600 hover:shadow-lg hover:shadow-red-600/25"
          >
            <Plus className="h-4 w-4" />
            Add Movie
          </Link>
          <Link
            href="/admin/import?type=movie"
      ...
      {selectedItems.size > 0 && (
        <div className="flex items-center gap-4 rounded-xl border border-red-500/30 bg-red-500/10 p-4">
          <span className="text-sm font-medium text-red-400">{selectedItems.size} selected</span>
          <button onClick={bulkDelete} className="flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700">
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-[#1a1a1a]/50 p-4 backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-black/40 pl-11 pr-4 py-2.5 text-sm text-white placeholder:text-gray-500 outline-none transition-all focus:border-red-500 focus:ring-2 focus:ring-red-500/20"
          />
        </div>
      ...
      {loading ? (
            ...
            <tr className="border-b border-white/10 bg-white/5">
              <th className="p-4 text-left">
                <input
                  ...
                  className="h-4 w-4 rounded border-gray-600 bg-black/40 text-red-600 focus:ring-red-500"
                />
              </th>
              <th className="p-4 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Movie</th>
              <th className="p-4 text-left text-xs font-medium text-gray-400 uppercase tracking-wider hidden md:table-cell">Year</th>
              <th className="p-4 text-left text-xs font-medium text-gray-400 uppercase tracking-wider hidden lg:table-cell">Language</th>
                ...
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 line-clamp-1">{item.quality}</p>
                    </div>
                  </div>
                </td>
                <td className="p-4 text-sm text-gray-300 hidden md:table-cell">{item.year}</td>
                <td className="p-4 text-sm text-gray-300 hidden lg:table-cell">{item.language}</td>
        ...
        {paginatedItems.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="mt-4 text-lg font-medium text-gray-400">No movies found</p>
            <p className="text-sm text-gray-600">Add your first movie or adjust your search</p>
          </div>
        )}
      </div>
      )}

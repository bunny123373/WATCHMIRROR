...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-gradient-to-b from-black/95 via-black/80 to-black/35 backdrop-blur">
      <div className="flex h-14 w-full items-center justify-between px-4 md:h-16 md:px-8">
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            <div className="hidden items-center gap-2 md:flex">
              <Link
                href="/my-list"
                className="flex items-center gap-1 rounded-lg p-2 text-gray-300 transition hover:text-white"
                title="My List"
              >
                <User size={18} />
              </Link>
            ...
            <Link
              href="/login"
              className="hidden rounded-lg bg-[#E50914] px-3 py-1.5 text-sm text-white transition hover:bg-[#b81d24] md:block"
            >
              Sign In
            </Link>
          )}
          ...
          <nav className="flex flex-col py-2">
            ...
            {user ? (
              <button
                ...
                className="flex items-center gap-2 px-4 py-3 text-sm font-medium text-gray-300 hover:bg-white/5"
              >
                <LogOut size={16} />
                Logout
              </button>
              ...
              <Link
                href="/login"
                onClick={() => setMenuOpen(false)}
                className="mx-4 mt-2 block rounded-lg bg-[#E50914] px-4 py-3 text-center text-sm font-medium text-white"
              >
                Sign In
              </Link>
            )}
